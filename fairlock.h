...
void fairlock_acquire(fairlock_t *lock, int idx) {
    flthread_info_t *info;
    ull now;
...
begin:
    ...
    while (1) {
        ...
        if (__sync_bool_compare_and_swap(&lock->qtail, prev, &n)) {
            ...
            now = rdtsc();
            // info->banned_until += (tot_children_at_level[H_node_adj[idx].level]-1)*CYCLE_PER_MS;
            info->start_ticks = now;
            info->slice = now + (H_node_adj[idx].slice* CYCLE_PER_MS);
...
void fairlock_release(fairlock_t *lock, int idx) {
    ull now, cs;
#ifdef DEBUG
